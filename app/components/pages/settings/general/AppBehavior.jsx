        ...
        >
            <CustomSection>
                <s-stack gap="small">
                    ...
                    />
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px',border: "1px solid", borderColor: billing?.isFree ? "#E0E0E0" : "transparent", padding: "4px 8px", borderRadius: "10px" }}>
                        <s-switch
                ...
                <s-stack gap="small" paddingBlockStart="base">
                    <s-stack>
                        <s-heading>Exchange rate update frequency {billing?.isFree && paidPlan(false)}</s-heading>
                    ...
                    <s-select value={exchangeRateFrequency} onChange={handleExchangeRateFrequency}>
                        {exchangeFrequency.map((item) => (
                            <s-option
                                ...
                                disabled={billing?.isFree && paidPlan(false) && item.value === "6_hours" || billing?.isFree && paidPlan(false) && item.value === "12_hours"}
                            >
                                {item.label}
                            </s-option>
                        ))}
