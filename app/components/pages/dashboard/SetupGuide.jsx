        ...
        <s-section>
          <s-stack gap="base">
            ...
            >
              ...
              >
                <CopyCurrencyEmbed
                  title="HTML with currency"
                  value={`<span class='money' data-amount='{{amount}}'>${`{{amount}}`} ${currencyFormats?.currencyCode}</span>`}
                />
                <CopyCurrencyEmbed
                  title="HTML without currency"
                  value={`<span class='money' data-amount='{{amount}}'>${`{{amount}}`}</span>`}
                />
              </s-grid>
              <s-stack paddingBlockStart="small">
                {hasCheckedMoneyFormat && !isMoneyFormatUpdated && (
                  <s-stack>
                    <s-query-container>
                      <s-grid
                        gridTemplateColumns="@container (inline-size > 490px) '1fr 1fr', 1fr"
                        gap="@container (inline-size > 490px) 'base large', small"
                      >
                        <s-banner
                        ...
                        >
                          ...
                          >
                            <s-text-field
                              ...
                              value={`${decodeHtml(currencyFormats?.currencyFormats?.moneyWithCurrencyFormat) || ""}`}
                            />
                            <s-text-field
                              readOnly
                              label="HTML without currency"
                              value={`${decodeHtml(currencyFormats?.currencyFormats?.moneyFormat) || ""}`}
                            />
                          </div>
                        </s-banner>
                        <s-banner
                          heading="Change to this money format"
                          tone="success"
                        >
                          <div
                            style={{
                          ...
                          >
                            <s-text-field
                              readOnly
                              label="HTML with currency"
                              value="<span class='money' data-amount='{{amount}}'>{{amount}} USD</span>"
                            />
                            <s-text-field
                              readOnly
                              label="HTML without currency"
