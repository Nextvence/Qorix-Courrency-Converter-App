...
import { defaultCurrencyDesign } from "../utils/default-settings";
...
export const loader = async ({ request }) => {
  ...
  const { currentAppInstallationId, metafieldMap } = await ensureAppMetafields(
    admin,
    ["currency_general", "currency_design", "settings_general"],
  );
  console.log(
    "Default Currency General",
    await getDefaultCurrencyGeneral(admin),
  );
  return {
    currentAppInstallationId,
    currencyGeneral:
      metafieldMap.currency_general || (await getDefaultCurrencyGeneral(admin)),
    currencyDesign: metafieldMap.currency_design || defaultCurrencyDesign,
    settingsGeneral:
      metafieldMap.settings_general || (await defaultSettingsGeneral(admin)),
...
export default function Currency() {
  ...
  const handleSave = () => {
    ...
    shopify.saveBar.hide("save-bar");
    shopify.toast.show("Settings saved", { duration: 2000 });
  };
  ...
  const handleChange = (event) => {
    if (event.target === "general") {
      ...
      console.log(
        "Something went wrong with the assigned target:",
        event.target,
      );
    }
  ...
  const handleToggleCurrencyStatus = () => {
    ...
    shopify.toast.show("Currency status updated", { duration: 2000 });
  };
  return (
          ...
          <s-stack direction="inline" gap="small" alignItems="center">
            <s-box>
              ...
              >
                ...
                <s-badge tone={isCurrencyEnabled ? "success" : "neutral"}>
                  {isCurrencyEnabled ? "On" : "Off"}
                </s-badge>
                <s-icon
                  type="question-circle"
                  interestFor="currency-status-info--tooltip"
                />
