...
export const loader = async ({ request }) => {
  ...
  const { currentAppInstallationId, metafieldMap } = await ensureAppMetafields(
    admin,
    ["settings_general", "settings_widget"],
  );
...
export default function Settings() {
  ...
  const handleClearVisitorPreferences = () => {
    const updatedGeneralSettings = {
      ...
      appBehavior: {
        ...
        visitorPreferenceVersion:
          (generalSettings.appBehavior?.visitorPreferenceVersion || 1) + 1,
      },
    };
    ...
    shopify.saveBar.hide("save-bar");
    shopify.toast.show("Visitor preferences cleared", { duration: 2000 });
  };
  ...
  const storeDefaultCurrency =
    currencyFormats?.shop?.currencyCode?.toLowerCase();
  return (
    <s-page heading={`${appName}`}>
      ...
      >
        ...
        <s-button
          variant="primary"
          icon="store"
          href={`https://${shop?.primaryDomain?.host}`}
          target="_blank"
        >
          View store
        </s-button>
      ...
      <s-section>
        ...
        {activeTab === "general" && (
          <CustomSection padding="none">
            <StoreDefaults
              data={{ generalSettings, storeDefaultCurrency }}
            ...
            <ExchangeRates
              data={{ generalSettings, exchangeMeta }}
              handleChange={handleChange}
            />
            {/* <s-stack paddingInlineEnd="large">
              <s-divider />
        ...
        {activeTab === "widget" && (
          <CustomSection>
            <WidgetPreview
              data={{ generalSettings, widgetSettings }}
              handleChange={handleChange}
            />
          </CustomSection>
        )}
